package
{
	...
	import TestBenches.TB_Starling;
	...
	public class Main_starling extends Sprite 
	{
		...
		private var ms:TB_Starling;
	
		
		public function Main_starling() 
		{
		...
		rController = new RenderController(Global.RENDER_STAGE_WIDTH, Global.RENDER_STAGE_HEIGHT, Global.renderCanvas);
			...
			rController.dispatcher.addEventListener(YTEvent.MOVIE_START, movStart);
			rController.dispatcher.addEventListener(YTEvent.MOVIE_STOP, movStop);
			rController.dispatcher.addEventListener(YTEvent.RENDER_START, renderStart);
			rController.dispatcher.addEventListener(YTEvent.RENDER_STOP, renderStop);
...
//!!			rendCanv.RENDER_CANVAS.addChild(t);
		...
		{
			rController.context = ms.context;
		
			
			Global.renderEnabled = true;
			trace("Render Started");
			
			trace(StarlingBase.t,"-------------------------------******_____________");
			StarlingBase.t.step();
		}
